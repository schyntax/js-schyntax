...
function getGroupEvent (group, start, mode)
{
	...
	for (var d = 0; d < (4*365); d++)
	{
		...
		var dayOfYear = Helpers.dayOfYear(date);
		...
		{
			applicable = false;
			for (i = 0; i < group.daysOfYear.length; i++)
			{
				if (inDayOfYearRange(group.daysOfYear[i], year, dayOfYear))
				{
					applicable = true;
					break;
				}
		...
		{
			for (i = 0; i < group.daysOfYearExcluded.length; i++)
			{
				if (inDayOfYearRange(group.daysOfYearExcluded[i], year, dayOfYear))
					continue date_loop;
			}
		}
 ...
 * @param year {number}
 * @param dayOfYear {number}
 * @return {boolean}
 */
function inDayOfYearRange (range, year, dayOfYear)
{
	if (range.start < 0 || (range.isRange && range.end < 0))
	{
		...
		var daysInYear = Helpers.isLeapYear(year) ? 366 : 365;
	...
	var daysInPrevYear = Helpers.IsLeapYear(year-1) ? 366 : 365;
