	...
	for (var i = 0; i < commands.length; i++)
	{
		...
		switch (name)
		{
			...
			case 'dates':
				...
				break;
			default:
				throw this.syntaxError('Unknown command ' + cmd.name, cmd.firstToken);
		}
		...
		for (x = 0; x < args.length; x++)
		{
			...
			if (arg.type === 'days')
			{
				if (propType !== 'daysOfWeek')
					throw this.syntaxError('Weekday names can only be used inside daysofweek commands.', arg.firstToken);
			...
			else if (arg.type === 'range')
			{
				...
				// Swap if low is greater than high, unless high is a negative and low is positive.
				...
				{
					low = arg.value.high | 0;
					high = arg.value.low | 0;
				}
			}
			else
			{
